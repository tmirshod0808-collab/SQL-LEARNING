...
--1Write a query that uses an alias to rename the ProductName column as Name in the Products table.
...
--3Use UNION to combine results from two queries that select ProductName from Products and ProductName from Products_Discounted.  

select ProductName from Products
union 
select ProductName from Products_Discounted

--4Write a query to find the intersection of Products and Products_Discounted tables using INTERSECT.

select ProductName from Products
intersect 
select ProductName from Products_Discounted

--5Write a query to select distinct customer names and their corresponding Country using SELECT DISTINCT.
 ...
 --7Use IIF to create a column that shows 'Yes' if StockQuantity > 100, and 'No' otherwise (Products_Discounted table, StockQuantity column).

 select StockQuantity, IIF (StockQuantity > 100 ,'yes','no') AS INSHORT from Products_Discounted
...
SELECT PRICE, IIF (PRICE> 1000, 'Expensive','Affordable') AS conditional FROM Products
...
--UNION
--SELECT AGE, SALARY FROM Employees WHERE Age < 25
...
--12Update the salary of an employee based on their department, increase by 10% if they work in 'HR' or EmployeeID = 5
 
UPDATE  EMPLOYEES  SET SALARY=SALARY * 1.10
WHERE DepartmentName = 'HR' OR EmployeeID = 5

--13Write a query that uses CASE to assign 'Top Tier' if SaleAmount > 500, 'Mid Tier' if SaleAmount BETWEEN 200 AND 500, and 'Low Tier' otherwise. (From Sales table)

SELECT SaleAmount, CASE 
WHEN  SaleAmount > 500 THEN 'Top Tier'
WHEN SALEAMOUNT BETWEEN 200 AND 500  THEN ' Mid Tier'
ELSE 'Low Tier' END AS TIERCATEGORY FROM Sales

--14Use EXCEPT to find customers' ID who have placed orders but do not have a corresponding record in the Sales table

--SELECT OrderID , CustomerID ,
--    ProductID FROM Orders
--EXCEPT 
--SELECT SALEID , CustomerID,
--    ProductID FROM SALES 
	SELECT CustomerID 
FROM Orders
EXCEPT
SELECT CustomerID 
